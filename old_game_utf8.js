    ...
    for (let row = 0; row < 10; row++) {
        for (let col = 0; col < 10; col++) {
            ...
            if (cardLabel === 'FREE') {
                ...
                cell.innerHTML = `<span style="font-size:2rem">Γÿà</span>`;
            } else {
                // Parse card
...
function updateStatus() {
    ...
    if (gameState.game_over) {
        ...
        if (gameState.winner === 1) {
            gameOverText.textContent = watchMode ? '≡ƒñû AI 1 Wins!' : '≡ƒÄë You Win!';
        } else if (gameState.winner === 2) {
            gameOverText.textContent = watchMode ? '≡ƒñû AI 2 Wins!' : '≡ƒñû AI Wins!';
        } else {
